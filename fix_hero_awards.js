...
// Fix: replace the broken span pattern with correct closing
const oldHeroBlock = `                <span className="relative">
...
const newHeroBlock = `                <span className="relative">
                  <span className="absolute -inset-3 rounded-full animate-pulse [animation-duration:2.2s] bg-[rgba(220,38,38,0.25)]" />
                  <span className="relative whitespace-nowrap">Enter the Dark Side</span>
              </Link>`;

// Check if fix_hero_precise already fixed it
if (hero.includes('</span>\n                </span>\n              </Link>')) {
  console.log('✅ Hero.tsx already fixed (double span close found)');
} else if (hero.includes('"relative whitespace-nowrap">Enter the Dark Side</span>\n              </Link>')) {
  // Broken: span opened but never closed before </Link>
  hero = hero.replace(
    '"relative whitespace-nowrap">Enter the Dark Side</span>\n              </Link>',
    '"relative whitespace-nowrap">Enter the Dark Side</span>\n                </span>\n              </Link>'
  );
  fs.writeFileSync(heroPath, hero, 'utf8');
  console.log('✅ Hero.tsx fixed');
} else {
  console.log('⚠️ Could not find the exact broken pattern in Hero.tsx');
}

// Verify spans balance
const heroOpenSpans = (hero.match(/<span\b[^>]*>/g) || []).length;
const heroCloseSpans = (hero.match(/<\/span>/g) || []).length;
console.log(`Hero spans - Open: ${heroOpenSpans}, Close: ${heroCloseSpans}, Diff: ${heroOpenSpans - heroCloseSpans}`);

// ===== REWRITE AWARDS.TSX =====
const awardsContent = `"use client";
    ...
    <section id="awards" ref={sectionRef} className="relative py-20 px-4 sm:px-6 overflow-hidden" style={{ background: "#0b0e14" }}>
      ...
      <div className="relative mx-auto max-w-7xl">
        ...
        <header className="text-center mb-12">
          <span className="inline-block text-[10px] uppercase tracking-[0.25em] font-black mb-3" style={{ color: "rgba(6,182,212,0.7)" }}>✦ AWARDS & RECOGNITION</span>
        ...
        {/* 3D Cinematic Film Reel Display */}
          ...
          <div className="flex items-center justify-center gap-4 sm:gap-6 overflow-x-auto py-8 px-2" style={{ transformStyle: "preserve-3d" }}>
            {FILM_FRAMES.map((item, idx) => {
              ...
              return (
                <motion.button
                  key={idx}
                  onClick={() => { setCurrentIdx(idx); openLightbox(idx); }}
                  className="relative flex-shrink-0 rounded-[16px] overflow-hidden cursor-pointer focus:outline-none focus:ring-2 focus:ring-cyan-400/50"
                  animate={{
                  ...
                  style={{
                    ...
                    border: isCenter ? "1.5px solid rgba(6,182,212,0.4)" : "1px solid rgba(148,163,184,0.1)",
                    boxShadow: isCenter
                      ? "0 0 50px rgba(6,182,212,0.15), 0 0 100px rgba(6,182,212,0.05), inset 0 0 30px rgba(6,182,212,0.03)"
                ...
                >
                  ...
                  <div className="absolute inset-x-0 bottom-0 p-4 sm:p-5 text-left z-10">
                    <span className="inline-block text-[9px] uppercase tracking-[0.22em] font-black px-2 py-1 rounded mb-2" style={{
            ...
            {FILM_FRAMES.map((_, idx) => (
              <button key={idx} onClick={() => setCurrentIdx(idx)}
                ...
                style={{
                  ...
                  height: "8px",
                  background: idx === currentIdx ? "#06b6d4" : "rgba(148,163,184,0.2)",
                  boxShadow: idx === currentIdx ? "0 0 12px rgba(6,182,212,0.4)" : "none",
                }}
...
console.log('✅ Awards.tsx completely rewritten with cinematic reel design');
</｜｜DSML｜｜parameter>
</create_file>
